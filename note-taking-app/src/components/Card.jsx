...
import { delNote, editNote } from "../redux/notesSlice";
...
function Card(item) {
  ...
  const handleEdit = () => {
    setEdit(true);
    dispatch(
      editNote({ id: item.item.id, note: value, color: item.item.color })
    );
    setSuccess("Saved!");
          ...
          <Flex justifyContent="space-between" bg="rgba(0,0,0,0.25)" px={1}>
            ...
            <Flex bg="none" my={1}>
              ...
              <Circle size="20px" bg="orange.400" color="white" ml={1}>
                <Icon
                  as={edit ? EditIcon : CheckIcon}
                  w={3.5}
                  bg="none"
                  onClick={() => (edit ? setEdit(false) : handleEdit())}
                />
              </Circle>
